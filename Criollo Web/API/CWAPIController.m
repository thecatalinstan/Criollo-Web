...
- (void)setupRoutes {
    ...
    CRRouteBlock checkAuthBlock = ^(CRRequest * _Nonnull request, CRResponse * _Nonnull response, CRRouteCompletionBlock  _Nonnull completionHandler) { @autoreleasepool {
    ...
    // Default route
    [self add:@"/" block:^(CRRequest * _Nonnull request, CRResponse * _Nonnull response, CRRouteCompletionBlock  _Nonnull completionHandler) { @autoreleasepool {
    ...
    // Login
    [self post:CWAPILoginPath block:^(CRRequest * _Nonnull request, CRResponse * _Nonnull response, CRRouteCompletionBlock  _Nonnull completionHandler) { @autoreleasepool {
        CWUser *user;
        NSDictionary<NSString *, NSString *> * credentials;
        NSMutableDictionary *payload;
        
        if (!(credentials = (NSDictionary *)request.body)) {
            goto error;
        }
        
        if (!(user = [CWUser authenticateWithUsername:credentials[CWAPIUsernameKey] password:credentials[CWAPIPasswordKey]])) {
            goto error;
        }

        [response setStatusCode:200 description:nil];
        [response setCookie:CWUserCookie value:[CWUser authenticationTokenForUser:user] path:@"/" expires:nil domain:nil secure:NO];
                        
        payload = user.toDictionary.mutableCopy;
    ...
    // Logout
    [self get:CWAPILogoutPath block:^(CRRequest * _Nonnull request, CRResponse * _Nonnull response, CRRouteCompletionBlock  _Nonnull completionHandler) { @autoreleasepool {
        [response setCookie:CWUserCookie value:@"deleted" path:@"/" expires:[NSDate distantPast] domain:nil secure:NO];
        [CWAPIController succeedWithPayload:nil request:request response:response];
    }}];

    // Currently authneticated user ("me")
    [self get:CWAPIMePath block:^(CRRequest * _Nonnull request, CRResponse * _Nonnull response, CRRouteCompletionBlock  _Nonnull completionHandler) { @autoreleasepool {
        CWUser * currentUser = [CWUser authenticatedUserForToken:request.cookies[CWUserCookie]];
        if ( currentUser ) {
    ...
    // Simple stack trace
    [self get:CWAPITracePath block:^(CRRequest * _Nonnull request, CRResponse * _Nonnull response, CRRouteCompletionBlock  _Nonnull completionHandler) { @autoreleasepool {
        [response sendData:[CWAPIResponse successResponseWithData:[NSThread callStackSymbols]].toJSONData];
    }}];

    // Env
    [self get:CWAPIEnvPath block:^(CRRequest * _Nonnull request, CRResponse * _Nonnull response, CRRouteCompletionBlock  _Nonnull completionHandler) { @autoreleasepool {
        [response sendData:[CWAPIResponse successResponseWithData:request.env].toJSONData];
    }}];

    // Info
    [self get:CWAPIInfoPath block:^(CRRequest * _Nonnull request, CRResponse * _Nonnull response, CRRouteCompletionBlock  _Nonnull completionHandler) { @autoreleasepool {
